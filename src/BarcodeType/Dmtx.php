...
class Dmtx extends InterleaveBarcodeType
{
    ...
    {
        list($data, $ec) = $this->encodeData($data, $rect, $fnc1);
        $data = $this->encodeEc($data, $ec);
        list($h, $w, $mtx) = $this->createMatrix($ec, $data);
        return [
            'g' => 'm',
            'q' => [1, 1, 1, 1],
            's' => [$w, $h],
     ...
     * @return array
     */
    private function encodeData($data, $rect, $fnc1): array
    {
        ...
        list($ec_params, $edata) = $this->addPadding($edata, $rect);
     ...
     * @return int[]
     */
    private function detectVersion($length, $rect): array
    {
        for ($i = ($rect ? 24 : 0), $j = ($rect ? 30 : 24); $i < $j; $i++) {
            if ($length <= self::EC_PARAMS[$i][0]) {
                return self::EC_PARAMS[$i];
            }
        }
        return self::EC_PARAMS[$j - 1];
    }
     ...
     * @return array
     */
    private function encodeEc($data, $ec_params): array
    {
        $blocks = $this->ecSplit($data, $ec_params);
        for ($i = 0, $n = count($blocks); $i < $n; $i++) {
            $ec_block = $this->ecDivide($blocks[$i], $ec_params, Dmtx::EC_POLYNOMIALS, Dmtx::LOG, Dmtx::EXP);
     ...
     * @return array
     */
    private function createMatrix($ec_params, $data): array
    {
        /* Create matrix. */
        ...
        list($rows, $cols, $matrix) = $this->createDataRegion($ec_params, $width);
        $this->placeData($matrix, $rows, $cols, $data);
        $bitmap = $this->copyIntoMatrix($ec_params, $matrix, $rheight, $rwidth, $bitmap);
        /* Return matrix. */
     ...
     * @return void
     */
    private function placeData(&$mtx, $rows, $cols, $data): void
    {
        $row = 4;
        $col = 0;
        $offset = 0;
        ...
        while (($row < $rows || $col < $cols) && $offset < $length) {
            ...
            if ($row == $rows && $col == 0) {
                $this->place1($mtx, $rows, $cols, $data[$offset++]);
            } elseif ($row == $rows - 2 && $col == 0 && $cols % 4 != 0) {
                $this->place2($mtx, $rows, $cols, $data[$offset++]);
            } elseif ($row == $rows - 2 && $col == 0 && $cols % 8 == 4) {
                $this->place3($mtx, $rows, $cols, $data[$offset++]);
            } elseif ($row == $rows + 4 && $col == 2 && $cols % 8 == 0) {
                $this->place4($mtx, $rows, $cols, $data[$offset++]);
            }
            $this->placeUpRightData($row, $col, $cols, $offset, $length, $rows, $mtx, $data, $b);
            $this->placeDownLeftData($row, $rows, $col, $offset, $length, $cols, $mtx, $data, $b);
        }
    }
     ...
     * @return void
     */
    private function place1(&$matrix, $rows, $cols, $b): void
    {
        $matrix[$rows - 1][0] = (($b & 0x80) ? 1 : 0);
        $matrix[$rows - 1][1] = (($b & 0x40) ? 1 : 0);
        $matrix[$rows - 1][2] = (($b & 0x20) ? 1 : 0);
        $matrix = $this->placeMatrix($cols, $b, $matrix);
    }
     ...
     * @return void
     */
    private function place2(&$matrix, $rows, $cols, $b): void
    {
        $matrix = $this->placeReverseMatrix($b, $matrix, $rows);
     ...
     * @return void
     */
    private function place3(&$matrix, $rows, $cols, $b): void
    {
        $matrix = $this->placeReverseMatrix($b, $matrix, $rows);
        $matrix = $this->placeMatrix($cols, $b, $matrix);
    }
     ...
     * @return void
     */
    private function place4(&$matrix, $rows, $cols, $b): void
    {
        $matrix[$rows - 1][        0] = (($b & 0x80) ? 1 : 0);
     ...
     * @return void
     */
    private function place0(&$matrix, $rows, $cols, $row, $col, $b): void
    {
        $this->placeB($matrix, $rows, $cols, $row-2, $col-2, $b & 0x80);
        $this->placeB($matrix, $rows, $cols, $row-2, $col-1, $b & 0x40);
        $this->placeB($matrix, $rows, $cols, $row-1, $col-2, $b & 0x20);
        $this->placeB($matrix, $rows, $cols, $row-1, $col-1, $b & 0x10);
        $this->placeB($matrix, $rows, $cols, $row-1, $col-0, $b & 0x08);
        $this->placeB($matrix, $rows, $cols, $row-0, $col-2, $b & 0x04);
        $this->placeB($matrix, $rows, $cols, $row-0, $col-1, $b & 0x02);
        $this->placeB($matrix, $rows, $cols, $row-0, $col-0, $b & 0x01);
    }
     ...
     * @return void
     */
    private function placeB(&$matrix, $rows, $cols, $row, $col, $b): void
    {
        if ($row < 0) {
            $row += $rows;
            $col += (4 - (($rows + 4) % 8));
     ...
     * @return array
     */
    private function placeReverseMatrix($b, $matrix, $rows): array
    {
     ...
     * @return array
     */
    private function placeMatrix($cols, $b, array $matrix): array
    {
        $matrix[0][$cols - 2] = (($b & 0x10) ? 1 : 0);
        $matrix[0][$cols - 1] = (($b & 0x08) ? 1 : 0);
     ...
     * @return array
     */
    private function createDataRegion($ec_params, float|int $width): array
    {
     ...
     * @return array
     */
    private function copyIntoMatrix($ec_params, mixed $matrix, int $rheight, int $rwidth, array $bitmap): array
    {
     ...
     * @return array
     */
    private function addPadding(array $edata, $rect): array
    {
        $length = count($edata);
        $ec_params = $this->detectVersion($length, $rect);
     ...
     * @return void
     */
    private function placeUpRightData(int &$row,
                                      int &$col,
                                      $cols,
                                      int &$offset,
                                      int $length,
                                          ...
                                          &$b): void
    {
        while ($row >= 0 && $col < $cols && $offset < $length) {
            if ($row < $rows && $col >= 0 && is_null($mtx[$row][$col])) {
                $b = $data[$offset++];
                $this->place0($mtx, $rows, $cols, $row, $col, $b);
            }
            $row -= 2;
            $col += 2;
        }
     ...
     * @return void
     */
    private function placeDownLeftData(int &$row,
                                           $rows,
                                       int &$col,
                                       int &$offset,
                                       int $length,
